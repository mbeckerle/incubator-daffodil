...
/**
 * Test rig that allows capture of heap dumps using jvisualVM.
 *
 * One test (the one with an xs:include) retains many Daffodil schema compiler objects
 * from daffodil-core.
 *
 * The other test (without any xs:include/import) does not retain these structures.
 * They are reclaimed by the GC.
 */
class TestForHeapDump {
  ...
  /**
   * Use to get a heap dump for this simple DFDL schema showing that
   * many schema-compiler data structures are retained, and visible on the HeapDump, including
   * DSOM objects, grammar Prod objects, etc.
   *
   * I suspect the validator is responsible for this (just a SWAG however)
   */
  // @Test
